...
#define P_ISEGHAL_SERVICE_CRTSUPPLY_STRING				"Supply"              	/* asynFloat64,        r */
...
#define P_ISEGHAL_SERVICE_MODSUPPLY_STRING				"Supply"              	/* asynFloat64,        r */
...
#define P_ISEGHAL_SERVICE_WNCHANTRIPACT_STRING				"TripAction"								/* asynFloat64,        r/w */, deprecated (use xxxTripAction instead)
...
/* @brief   asynPortDriver for ISEG & WIENER HV PS using iseghal service library
	*
	* This asynPortDriver is the device support for ISEG Spezialelektronik GmbH HV PS system using
	* the MMC/MICC controller (via iCSmini 2 controller box) & MMS-CONTROLLER (via CC24 embedded controller) using IsegHAL-Service Library.
*/
class drvAsynIseghalService : public asynPortDriver {
	public:
		drvAsynIseghalService( const char *portName, const char *interface, bool mpod );
...
#define NUM_ISEGHAL_SERVICE_PARAMS (&LAST_ISEGHAL_SERVICE_CMD - &FIRST_ISEGHAL_SERVICE_CMD + 1)
